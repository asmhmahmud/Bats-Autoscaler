 ...
 * @author Hasan
 */
public class BATSHV extends Algorithm
{
        ...
        private Decision startReactiveSlot(int slotNo, ReactiveDecision reactiveDecision)
        {
                ...
                if (slotNo > 0)
                {
                        initDelay = runDelay.getDelay(reactiveDecision.getVmNumbers().getTotalVM(), (int) expInput.noOfClients, reactiveDecision.getVmNumbers().getNonZeroVMType());
        ...
        protected void optimizeDecission(int slotNo, Decision decission, int currentWorkload)
        {
                ...
                else
                {
                        ...
                        queueLength[slotNo + 1] = Math.max(0, queueLength[slotNo] + decission.getTotalCost() - hourlyBudget[slotNo]);
        ...
        protected void getOptimalVMInstances(int currentWorkload, double currentQueueLength, double V, Decision decission)
        {
                ...
                delay = runDelay.getDelay(Settings.MAX_VM, currentWorkload, VMType.MEDIUM);
                decission.addVMNumberDecission(VMType.MEDIUM, Settings.MAX_VM);
                //System.out.println("Expct Delay1: "+delay);
                decission.setExpectedDelay(delay);
                int currentVMTypeIndex = 0;
                for (currentVMTypeIndex = 0; currentVMTypeIndex < VMType.totalVMType(); currentVMTypeIndex++)
                {
                        VMType vmType = VMType.getVMTypeByTypeIndex(currentVMTypeIndex);
                        for (int currentVM = 1; currentVM <= Settings.MAX_VM; currentVM++)
                        {
                                delay = runDelay.getDelay(currentVM, currentWorkload, currentVMTypeIndex);
                                ...
                                {
                                        decission.getVmNumbers().clearVMNumbers();
                                        optVal = currentVal;
                ...
                {
                        //System.out.println("Pre Decission: "+decission.getVmNumbers().getVMDecisionNumbers()+", Delay: "+decission.getExpectedDelay());

                        int currentVM = decission.getVmNumbers().getTotalVM() + 1;
                        currentVMTypeIndex = VMType.getVMTypeIndexByVMType(decission.getVmNumbers().getNonZeroVMType());
                        for (; currentVMTypeIndex < VMType.totalVMType(); currentVMTypeIndex++)
                        {
                                //System.out.println(currentVMTypeIndex);
                                VMType vmType = VMType.getVMTypeByTypeIndex(currentVMTypeIndex);
                                for (; currentVM < Settings.MAX_VM; currentVM++)
                                {
                ...
                System.out.println("Decission: " + decission.getVmNumbers().getVMDecisionNumbers());
